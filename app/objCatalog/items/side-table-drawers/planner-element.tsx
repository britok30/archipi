...
export default {
  ...
  info: {
    title: "Side Table with Drawers",
    tag: ["furnishings","living","bedroom"],
    description: "Side table with drawers",
    image: "/images/side-table-drawers.png",
  },
  ...
  render2D: function (element: any, layer: any, scene: any) {
    let newWidth = readItemLength(element.properties?.width, scene, 60);

    let newDepth = readItemLength(element.properties?.depth, scene, 25);
  ...
  render3D: async function (element: any, layer: any, scene: any) {
    const newWidth = readItemLength(element.properties?.width, scene, 60);

    const newDepth = readItemLength(element.properties?.depth, scene, 25);

    const newHeight = readItemLength(element.properties?.height, scene, 55);

    const newAltitude = readItemLength(element.properties?.altitude, scene, 0);

    const item = new Three.Object3D();
    const model = await loadGlb("/models/side-table-drawers.glb");
