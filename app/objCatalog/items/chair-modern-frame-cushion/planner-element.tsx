...
export default {
  ...
  info: {
    title: "Modern Frame Chair",
    tag: ["furnishings","living","kitchen"],
    description: "Modern frame chair with cushion",
    image: "/images/chair-modern-frame-cushion.png",
  },
  ...
  render2D: function (element: any, layer: any, scene: any) {
    let newWidth = readItemLength(element.properties?.width, scene, 45);

    let newDepth = readItemLength(element.properties?.depth, scene, 50);
  ...
  render3D: async function (element: any, layer: any, scene: any) {
    const newWidth = readItemLength(element.properties?.width, scene, 45);

    const newDepth = readItemLength(element.properties?.depth, scene, 50);
    ...
    const model = await loadGlb("/models/chair-modern-frame-cushion.glb");
