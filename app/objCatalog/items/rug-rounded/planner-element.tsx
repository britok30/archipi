...
export default {
  ...
  info: {
    title: "Rounded Rug",
    tag: ["furnishings","decor","living"],
    description: "Rug with rounded corners",
    image: "/images/rug-rounded.png",
  },
  ...
  render2D: function (element: any, layer: any, scene: any) {
    let newWidth = readItemLength(element.properties?.width, scene, 200);

    let newDepth = readItemLength(element.properties?.depth, scene, 140);
  ...
  render3D: async function (element: any, layer: any, scene: any) {
    const newWidth = readItemLength(element.properties?.width, scene, 200);

    const newDepth = readItemLength(element.properties?.depth, scene, 140);

    const newHeight = readItemLength(element.properties?.height, scene, 1);

    const newAltitude = readItemLength(element.properties?.altitude, scene, 0);

    const item = new Three.Object3D();
    const model = await loadGlb("/models/rug-rounded.glb");
