...
export default {
  ...
  info: {
    title: "Small Leafy Plant",
    tag: ["furnishings","decor"],
    description: "Small leafy potted plant",
    image: "/images/plant-small-3.png",
  },
  ...
  render2D: function (element: any, layer: any, scene: any) {
    let newWidth = readItemLength(element.properties?.width, scene, 25);
  ...
  render3D: async function (element: any, layer: any, scene: any) {
    const newWidth = readItemLength(element.properties?.width, scene, 25);

    const newDepth = readItemLength(element.properties?.depth, scene, 25);

    const newHeight = readItemLength(element.properties?.height, scene, 45);

    const newAltitude = readItemLength(element.properties?.altitude, scene, 0);

    const item = new Three.Object3D();
    const model = await loadGlb("/models/plant-small-3.glb");
