...
export default {
  ...
  render2D: function (element: any, layer: any, scene: any) {
    let newWidth = readItemLength(element.properties?.width, scene, 8);
  ...
  render3D: async function (element: any, layer: any, scene: any) {
    const newWidth = readItemLength(element.properties?.width, scene, 8);

    const newDepth = readItemLength(element.properties?.depth, scene, 12);

    const newHeight = readItemLength(element.properties?.height, scene, 4);

    const newAltitude = readItemLength(element.properties?.altitude, scene, 75);

    const item = new Three.Object3D();
    const model = await loadGlb("/models/computer-mouse.glb");
