...
export default {
  ...
  info: {
    title: "Wall Lamp",
    tag: ["furnishings","decor"],
    description: "Wall-mounted lamp",
    image: "/images/lamp-wall.png",
  },
  ...
  render2D: function (element: any, layer: any, scene: any) {
    let newWidth = readItemLength(element.properties?.width, scene, 20);

    let newDepth = readItemLength(element.properties?.depth, scene, 12);
  ...
  render3D: async function (element: any, layer: any, scene: any) {
    const newWidth = readItemLength(element.properties?.width, scene, 20);

    const newDepth = readItemLength(element.properties?.depth, scene, 12);

    const newHeight = readItemLength(element.properties?.height, scene, 10);

    const newAltitude = readItemLength(element.properties?.altitude, scene, 180);

    const item = new Three.Object3D();
    const model = await loadGlb("/models/lamp-wall.glb");
