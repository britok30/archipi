...
const pointsDistance = (x1: any, y1: any, x2: any, y2: any) => {
    ...
    if (target.nodeName === "circle") {
      if (target.attributes.name) {
        if (target.attributes.name.nodeValue === "fst-anchor") {
          setState({ ...state, handleMouseMove1: !state.handleMouseMove1 });
        } else if (target.attributes.name.nodeValue === "snd-anchor") {
          setState({ ...state, handleMouseMove2: !state.handleMouseMove2 });
        }
      }
    }
  };
  ...
  const onMouseMove = (event: any) => {
    ...
    if (state.handleMouseMove1) {
      ...
      projectActions.setProperties(
        Map({ x1: newX, y1: newY, distance: Map({ length: dist }) })
      );
    } else if (state.handleMouseMove2) {
      let dist = pointsDistance(x1, y1, newX, newY);
      projectActions.setProperties(
        Map({ x2: newX, y2: newY, distance: Map({ length: dist }) })
      );
    }
  };

  const toggleHover1 = (e: any) => {
    setState((prevState) => ({ ...prevState, hover1: !prevState.hover1 }));
  };

  const toggleHover2 = (e: any) => {
  ...
  useEffect(() => {
    ...
    if (imageUri) {
      ...
      img.onload = () => {
        ...
        projectActions.setProperties(
          Map({ width: img.naturalWidth, height: img.naturalHeight })
        );
      };
      img.onerror = () => {
