...
export default {
  ...
  info: {
    title: "Antenna TV",
    tag: ["furnishings","living","decor"],
    description: "Retro television with antenna",
    image: "/images/television-antenna.png",
  },
  ...
  render3D: async function (element: any, layer: any, scene: any) {
    ...
    const newHeight = readItemLength(element.properties?.height, scene, 15);

    const newAltitude = readItemLength(element.properties?.altitude, scene, 0);

    const item = new Three.Object3D();
    const model = await loadGlb("/models/television-antenna.glb");
