...
export default {
  ...
  render2D: function (element: any, layer: any, scene: any) {
    let newWidth = readItemLength(element.properties?.width, scene, 35);
  ...
  render3D: async function (element: any, layer: any, scene: any) {
    const newWidth = readItemLength(element.properties?.width, scene, 35);

    const newDepth = readItemLength(element.properties?.depth, scene, 25);

    const newHeight = readItemLength(element.properties?.height, scene, 25);

    const newAltitude = readItemLength(element.properties?.altitude, scene, 75);

    const item = new Three.Object3D();
    const model = await loadGlb("/models/laptop.glb");
