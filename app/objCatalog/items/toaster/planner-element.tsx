...
export default {
  ...
  info: {
    ...
    description: "Countertop toaster",
  ...
  render3D: async function (element: any, layer: any, scene: any) {
    ...
    const newHeight = readItemLength(element.properties?.height, scene, 20);

    const newAltitude = readItemLength(element.properties?.altitude, scene, 90);

    const item = new Three.Object3D();
    const model = await loadGlb("/models/toaster.glb");
