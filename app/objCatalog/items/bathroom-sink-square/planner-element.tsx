...
export default {
  ...
  info: {
    title: "Square Bathroom Sink",
    tag: ["furnishings","bathroom"],
    description: "Square bathroom sink",
    image: "/images/bathroom-sink-square.png",
  },
  ...
  render3D: async function (element: any, layer: any, scene: any) {
    ...
    const newHeight = readItemLength(element.properties?.height, scene, 85);

    const newAltitude = readItemLength(element.properties?.altitude, scene, 0);

    const item = new Three.Object3D();
    const model = await loadGlb("/models/bathroom-sink-square.glb");
