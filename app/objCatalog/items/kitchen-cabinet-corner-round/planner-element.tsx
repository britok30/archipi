...
export default {
  ...
  info: {
    title: "Rounded Corner Kitchen Cabinet",
    tag: ["furnishings","kitchen"],
    description: "Rounded corner kitchen cabinet",
    image: "/images/kitchen-cabinet-corner-round.png",
  },
  ...
  render2D: function (element: any, layer: any, scene: any) {
    let newWidth = readItemLength(element.properties?.width, scene, 90);

    let newDepth = readItemLength(element.properties?.depth, scene, 90);
  ...
  render3D: async function (element: any, layer: any, scene: any) {
    const newWidth = readItemLength(element.properties?.width, scene, 90);

    const newDepth = readItemLength(element.properties?.depth, scene, 90);

    const newHeight = readItemLength(element.properties?.height, scene, 90);

    const newAltitude = readItemLength(element.properties?.altitude, scene, 0);

    const item = new Three.Object3D();
    const model = await loadGlb("/models/kitchen-cabinet-corner-round.glb");
