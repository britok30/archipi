  ...
  let wallElement = {
    ...
    calculatePolygonPoints: function (
      ...
      if (lines1.size === 0) {
        ...
        if (Math.abs(angleDiff) < THRESHOLD_ANGLE || lengthLine < EPSILON) {
          ...
          Math.abs(angleDiff) > THRESHOLD_ANGLE &&
          Math.abs(Math.abs(angleDiff) - Math.PI / 2) > EPSILON_ANGLE
        ) {
          let topWall = Geometry.linePassingThroughTwoPoints(
            0,
            half_thickness,
            length,
            half_thickness
          );
          let botLine = Geometry.linePassingThroughTwoPoints(
            0,
            -half_thickness / Math.cos(angleDiff),
            lengthLine * Math.cos(angleDiff),
            lengthLine * Math.sin(angleDiff) -
              half_thickness / Math.cos(angleDiff)
          );
          point = Geometry.twoLinesIntersection(
            topWall.a,
            topWall.b,
            topWall.c,
            botLine.a,
          ...
          let botWall = Geometry.linePassingThroughTwoPoints(
            0,
            -half_thickness,
            length,
            -half_thickness
          );
          let topLine = Geometry.linePassingThroughTwoPoints(
            0,
            half_thickness / Math.cos(angleDiff),
            lengthLine * Math.cos(angleDiff),
            lengthLine * Math.sin(angleDiff) +
          ...
          let topLine = Geometry.linePassingThroughTwoPoints(
            0,
            half_thickness / Math.sin(angleDiff - Math.PI / 2),
            -lengthLine * Math.sin(angleDiff - Math.PI / 2),
            lengthLine * Math.cos(angleDiff - Math.PI / 2) +
              half_thickness / Math.sin(angleDiff - Math.PI / 2)
          );
          point = Geometry.twoLinesIntersection(
            topWall.a,
        ...
        const angles = points.map((point) =>
          Geometry.angleBetweenTwoVertices(wp1, point)
        );
        const angleDiffs = angles.map((angle) => angle - angleWall1_2);
        const lengths = points.map((point) =>
          Geometry.verticesDistance(wp1, point)
        );

        const { min, max, minIdx, maxIdx } = angleDiffs.reduce(
          (acc, angle, idx) => {
            if (angle < 0) {
              angle += 2 * Math.PI;
            }
        ...
        if (Math.abs(minAngleDiff) < EPSILON_ANGLE || minLength < EPSILON) {
          ...
          Math.abs(minAngleDiff) > EPSILON_ANGLE &&
          Math.abs(minAngleDiff) < Math.PI / 2 - EPSILON_ANGLE
        ) {
          let topWall = Geometry.linePassingThroughTwoPoints(
            0,
            half_thickness,
          ...
          let botLine = Geometry.linePassingThroughTwoPoints(
            0,
            -half_thickness / Math.cos(minAngleDiff),
            minLength * Math.cos(minAngleDiff),
            minLength * Math.sin(minAngleDiff) -
              half_thickness / Math.cos(minAngleDiff)
          );
          point = Geometry.twoLinesIntersection(
            topWall.a,
          ...
          Math.abs(Math.abs(minAngleDiff) - Math.PI / 2) < EPSILON_ANGLE
        ) {
          poly_points.push({ x: half_thickness, y: half_thickness });
        } else if (
          Math.abs(minAngleDiff) > Math.PI / 2 + EPSILON_ANGLE &&
          Math.abs(minAngleDiff) < Math.PI - EPSILON_ANGLE
        ) {
          let topWall = Geometry.linePassingThroughTwoPoints(
            0,
            half_thickness,
          ...
          let topLine = Geometry.linePassingThroughTwoPoints(
            0,
            half_thickness / Math.sin(minAngleDiff - Math.PI / 2),
            -minLength * Math.sin(minAngleDiff - Math.PI / 2),
            minLength * Math.cos(minAngleDiff - Math.PI / 2) +
              half_thickness / Math.sin(minAngleDiff - Math.PI / 2)
          );
          point = Geometry.twoLinesIntersection(
            topWall.a,
          ...
          Math.abs(minAngleDiff) > Math.PI + EPSILON_ANGLE &&
          Math.abs(minAngleDiff) < (3 * Math.PI) / 2 - EPSILON_ANGLE
        ) {
          let topWall = Geometry.linePassingThroughTwoPoints(
            0,
            half_thickness,
          ...
          Math.abs(Math.abs(minAngleDiff) - (3 * Math.PI) / 2) < EPSILON_ANGLE
        ) {
          poly_points.push({ x: -half_thickness, y: half_thickness });
        } else if (
          Math.abs(minAngleDiff) > (3 * Math.PI) / 2 + EPSILON_ANGLE &&
          Math.abs(minAngleDiff) < 2 * Math.PI - EPSILON_ANGLE
        ) {
          let topWall = Geometry.linePassingThroughTwoPoints(
            0,
            half_thickness,
        ...
        if (Math.abs(maxAngleDiff) < EPSILON_ANGLE || maxLength < EPSILON) {
          ...
          Math.abs(maxAngleDiff) > EPSILON_ANGLE &&
          Math.abs(maxAngleDiff) < Math.PI / 2 - EPSILON_ANGLE
        ) {
          let botWall = Geometry.linePassingThroughTwoPoints(
            0,
            -half_thickness,
          ...
          let topLine = Geometry.linePassingThroughTwoPoints(
            0,
            half_thickness / Math.cos(maxAngleDiff),
            maxLength * Math.cos(maxAngleDiff),
            maxLength * Math.sin(maxAngleDiff) +
              half_thickness / Math.cos(maxAngleDiff)
          );
          point = Geometry.twoLinesIntersection(
            botWall.a,
          ...
          Math.abs(Math.abs(maxAngleDiff) - Math.PI / 2) < EPSILON_ANGLE
        ) {
          poly_points.push({ x: half_thickness, y: -half_thickness });
        } else if (
          Math.abs(maxAngleDiff) > Math.PI / 2 + EPSILON_ANGLE &&
          Math.abs(maxAngleDiff) < Math.PI - EPSILON_ANGLE
        ) {
          let botWall = Geometry.linePassingThroughTwoPoints(
            0,
            -half_thickness,
          ...
          let botLine = Geometry.linePassingThroughTwoPoints(
            0,
            half_thickness / Math.sin(Math.PI / 2 - maxAngleDiff),
            maxLength * Math.sin(Math.PI / 2 - maxAngleDiff),
            maxLength * Math.cos(Math.PI / 2 - maxAngleDiff) -
              half_thickness / Math.sin(Math.PI / 2 - maxAngleDiff)
          );
          point = Geometry.twoLinesIntersection(
            botWall.a,
          ...
          Math.abs(maxAngleDiff) > Math.PI + EPSILON_ANGLE &&
          Math.abs(maxAngleDiff) < (3 * Math.PI) / 2 - EPSILON_ANGLE
        ) {
          let botWall = Geometry.linePassingThroughTwoPoints(
            0,
            -half_thickness,
          ...
          let botLine = Geometry.linePassingThroughTwoPoints(
            0,
            -half_thickness / Math.sin(maxAngleDiff - Math.PI / 2),
            maxLength * Math.sin(maxAngleDiff - Math.PI / 2),
            maxLength * Math.cos(maxAngleDiff - Math.PI / 2) -
              half_thickness / Math.sin(maxAngleDiff - Math.PI / 2)
          );
          point = Geometry.twoLinesIntersection(
            botWall.a,
          ...
          Math.abs(Math.abs(maxAngleDiff) - (3 * Math.PI) / 2) < EPSILON_ANGLE
        ) {
          poly_points.push({ x: -half_thickness, y: -half_thickness });
        } else if (
          Math.abs(maxAngleDiff) > (3 * Math.PI) / 2 + EPSILON_ANGLE &&
          Math.abs(maxAngleDiff) < 2 * Math.PI - EPSILON_ANGLE
        ) {
          let botWall = Geometry.linePassingThroughTwoPoints(
            0,
            -half_thickness,
      ...
      if (lines2.size === 0) {
        ...
        if (Math.abs(angleDiff) < THRESHOLD_ANGLE || lengthLine < EPSILON) {
          ...
          let topLine = Geometry.linePassingThroughTwoPoints(
            length,
            half_thickness / Math.cos(angleDiff),
            length - lengthLine * Math.cos(angleDiff),
            -lengthLine * Math.sin(angleDiff) +
              half_thickness / Math.cos(angleDiff)
          );
          let concavePoint = Geometry.twoLinesIntersection(
            botWall.a,
            botWall.b,
            botWall.c,
            topLine.a,
          ...
          let convexPoint = Geometry.twoLinesIntersection(
            topWall.a,
            topWall.b,
            topWall.c,
            botLine.a,
          ...
          let botLine = Geometry.linePassingThroughTwoPoints(
            length,
            -half_thickness / Math.sin(angleDiff - Math.PI / 2),
            length - lengthLine * Math.sin(angleDiff - Math.PI / 2),
            lengthLine * Math.cos(angleDiff - Math.PI / 2) -
              half_thickness / Math.sin(angleDiff - Math.PI / 2)
          );
          let concavePoint = Geometry.twoLinesIntersection(
        ...
        const angles = points.map((point) =>
          Geometry.angleBetweenTwoVertices(wp2, point)
        );
        const angleDiffs = angles.map((angle) => angle - angleWall2_1);
        const lengths = points.map((point) =>
          Geometry.verticesDistance(wp2, point)
        );
        ...
        if (Math.abs(minAngleDiff) < EPSILON_ANGLE || minLength < EPSILON) {
          ...
          let topLine = Geometry.linePassingThroughTwoPoints(
            length,
            half_thickness / Math.cos(minAngleDiff),
            length - minLength * Math.cos(minAngleDiff),
            -minLength * Math.sin(minAngleDiff) +
              half_thickness / Math.cos(minAngleDiff)
          );
          point = Geometry.twoLinesIntersection(
          ...
          let botLine = Geometry.linePassingThroughTwoPoints(
            length,
            -half_thickness / Math.sin(minAngleDiff - Math.PI / 2),
            length - minLength * Math.sin(minAngleDiff - Math.PI / 2),
            minLength * Math.cos(minAngleDiff - Math.PI / 2) -
              half_thickness / Math.sin(minAngleDiff - Math.PI / 2)
          );
          point = Geometry.twoLinesIntersection(
        ...
        if (Math.abs(maxAngleDiff) < EPSILON_ANGLE || maxLength < EPSILON) {
          ...
          let botLine = Geometry.linePassingThroughTwoPoints(
            length,
            -half_thickness / Math.cos(maxAngleDiff),
            length - maxLength * Math.cos(maxAngleDiff),
            -maxLength * Math.sin(maxAngleDiff) -
              half_thickness / Math.cos(maxAngleDiff)
          );
          point = Geometry.twoLinesIntersection(
          ...
          let topLine = Geometry.linePassingThroughTwoPoints(
            length,
            half_thickness / Math.sin(maxAngleDiff - Math.PI / 2),
            length - maxLength * Math.sin(maxAngleDiff - Math.PI / 2),
            maxLength * Math.cos(maxAngleDiff - Math.PI / 2) +
              half_thickness / Math.sin(maxAngleDiff - Math.PI / 2)
          );
          point = Geometry.twoLinesIntersection(
    ...
    render2D: function (element, layer, scene) {
      ...
      let {
        ...
        y: y1,
        lines: lines1,
      } = layer.vertices.get(element.vertices.get(0));
      let {
        x: x2,
        y: y2,
        lines: lines2,
      } = layer.vertices.get(element.vertices.get(1));
      ...
      const poly_points = this.calculatePolygonPoints(
        element,
        layer,
        length,
        lines1,
